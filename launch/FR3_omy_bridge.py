...
- OMY-base-frame clutch-relative target mapping
- cumulative unfiltered Cartesian command pose
...
POSITION_SCALE = 0.30
ORIENTATION_SCALE = 1.00  # 1:1 rotation; 30 deg + 30 deg -> 60 deg
...
def rotation_distance_deg(reference_rotation, current_rotation):
    """Magnitude of the spatial rotation from reference to current."""
    relative_rotation = (
        current_rotation
    ...
    return float(
        np.degrees(
            np.linalg.norm(
                matrix_to_rotvec(relative_rotation)
            )
        )
    )
...
def make_desired_target(
    ...
    """Create a cumulative FR3 command from OMY base-frame increments.

    Position:
        The MuJoCo site position is expressed in the model world frame. When
        the robot base is fixed and aligned with that world frame, this is the
        robot-base-frame displacement.

    Orientation:
        R_current @ R_anchor.T is a spatial/base-frame relative rotation.
        The mapped increment is therefore left-multiplied onto the FR3 anchor.

        This gives the intended clutch behavior:
        +30 deg, release, return the leader, re-clutch, +30 deg -> +60 deg.
    """
    delta_position_omy_base = (
        omy_current_position
        - omy_anchor_position
    )
    desired_position = (
        fr3_anchor_position
        + POSITION_SCALE * (
            AXIS_MAP @ delta_position_omy_base
        )
    )

    # Spatial/base-frame relative rotation, not EE-local/body rotation.
    omy_delta_rotation_base = (
        omy_current_rotation
        @ omy_anchor_rotation.T
    )
    omy_delta_rotvec_base = matrix_to_rotvec(
        omy_delta_rotation_base
    )

    mapped_delta_rotvec_base = (
        ORIENTATION_SCALE
        * (
            ROTATION_VECTOR_MAP
            @ omy_delta_rotvec_base
        )
    )
    fr3_delta_rotation_base = rotvec_to_matrix(
        mapped_delta_rotvec_base
    )

    # Left multiplication applies the increment about fixed FR3 base axes.
    desired_rotation = (
        fr3_delta_rotation_base
        @ fr3_anchor_rotation
    )

    mapping_diagnostics = {
        "omy_delta_position_m": float(
            np.linalg.norm(delta_position_omy_base)
        ),
        "omy_delta_rotation_deg": float(
            np.degrees(np.linalg.norm(omy_delta_rotvec_base))
        ),
        "mapped_delta_rotation_deg": float(
            np.degrees(np.linalg.norm(mapped_delta_rotvec_base))
        ),
    }
    return desired_position, desired_rotation, mapping_diagnostics
...
def main():
    ...
    # Logical cumulative Cartesian command:
    # updated directly from each clutch stroke and never reduced by rate-limit
    # lag. The conditioned target below follows this command smoothly.
    fr3_command_position = fr3_target_position.copy()
    fr3_command_rotation = fr3_target_rotation.copy()

    fr3_session_initial_position = fr3_command_position.copy()
    fr3_session_initial_rotation = fr3_command_rotation.copy()
    ...
    # Re-clutch anchors use the cumulative command pose, not actual EE pose
    # and not the rate-limited target pose.
    fr3_anchor_position = fr3_command_position.copy()
    fr3_anchor_rotation = fr3_command_rotation.copy()

    mapping_diagnostics = {
        "omy_delta_position_m": 0.0,
        "omy_delta_rotation_deg": 0.0,
        "mapped_delta_rotation_deg": 0.0,
    }
    ...
    log_path = ROOT / "logs" / f"base_frame_cumulative_{run_id}.csv"
        ...
        while keep_running():
            ...
            if ros_fresh:
                ...
                if trigger_rising_edge:
                    ...
                    # Leader anchor: current OMY EE pose in OMY base/world.
                    omy_anchor_position = omy_current_position.copy()
                    omy_anchor_rotation = omy_current_rotation.copy()

                    # Follower anchor: cumulative logical Cartesian command.
                    # This preserves +30 deg +30 deg = +60 deg even when the
                    # rate-limited target or actual FR3 has not fully caught up.
                    fr3_anchor_position = fr3_command_position.copy()
                    fr3_anchor_rotation = fr3_command_rotation.copy()
                    ...
                    anchor_angle_deg = rotation_distance_deg(
                        fr3_session_initial_rotation,
                        fr3_anchor_rotation,
                    )
                    print(
                        f"Teleoperation ON | clutch={clutch_id} | "
                        f"FR3 command anchor={anchor_angle_deg:.2f} deg"
                    )

                elif trigger_falling_edge:
                    teleop_active = False
                    target_angular_velocity = np.zeros(3)

                    command_angle_deg = rotation_distance_deg(
                        fr3_session_initial_rotation,
                        fr3_command_rotation,
                    )
                    print(
                        f"Teleoperation OFF | clutch={clutch_id} | "
                        f"held cumulative command={command_angle_deg:.2f} deg"
                    )
                ...
                # The rate-limited target continues converging to the held
                # cumulative command after trigger release. Leader return
                # motion is ignored while teleop_active is False.
                (
                    fr3_target_position,
                    fr3_target_rotation,
                    target_linear_speed,
                ...
                # Safety behavior on stale ROS input: freeze the logical
                # command at the currently conditioned target.
                fr3_command_position = fr3_target_position.copy()
                fr3_command_rotation = fr3_target_rotation.copy()
            ...
            if (
                ...
                rows.append({
                    ...
                    "clutch_id": clutch_id,
                    "omy_delta_position_m": mapping_diagnostics[
                        "omy_delta_position_m"
                    ],
                    "omy_delta_rotation_deg": mapping_diagnostics[
                        "omy_delta_rotation_deg"
                    ],
                    "mapped_delta_rotation_deg": mapping_diagnostics[
                        "mapped_delta_rotation_deg"
                    ],
                    "fr3_anchor_cumulative_rotation_deg":
            ...
            if now - last_print >= 1.0 / PRINT_HZ:
                ...
                target_angle_deg = rotation_distance_deg(
                    fr3_session_initial_rotation,
                    fr3_target_rotation,
                )
                print(
                    f"clutch={clutch_id} | "
                    f"OMY stroke={mapping_diagnostics['omy_delta_rotation_deg']:.1f} deg | "
                    f"FR3 command={command_angle_deg:.1f} deg | "
                    f"target={target_angle_deg:.1f} deg | "
                    f"EE error={diagnostics['orientation_error_deg']:.1f} deg | "
